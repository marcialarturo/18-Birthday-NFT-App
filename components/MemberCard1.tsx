...
const MemberCard1 = ({ contact }: any) => {

  const { memberName, donationBalance, userWallet: user_wallet, user_avatar } = contact;
  const { setSelectedProfile } = useContext(AppContext);
  const router = useRouter();

  const handleSelectRpofile = () => {
    setSelectedProfile(contact);
    router.push(`/member-details/${user_wallet}`);
  };
        ...
        <div className="flex items-center">
          ...
          <div className="flex items-center">
            <p className="text-lightGray text-sm leading-4 font-normal text-right  mr-2">
              Total donations: {donationBalance.toString()}
            </p>
